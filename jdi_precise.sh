...
APPZ_REPO_UFFICIALI="avant-window-navigator awn-applets-all banshee banshee-community-extensions build-essential bwm-ng cmatrix compizconfig-settings-manager conky ffmpeg filezilla finch fusion-icon gdebi gimp gmail-notify gnome-commander gnome-mplayer gnome-search-tool gnome-session-fallback gnome-system-tools gnome-tweak-tool gparted gthumb gtk-recordmydesktop hardinfo hddtemp htop icedtea-plugin iotop k3b libreoffice lm-sensors mc meld mercurial mplayer mumble myunity nautilus-actions nautilus-image-converter nautilus-open-terminal nfs-common nfs-kernel-server p7zip p7zip-full p7zip-rar pidgin powertop pwgen rar sloccount smartmontools soundconverter sox subtitleeditor subversion synaptic tagtool tofrodos tree ttf-mscorefonts-installer ubuntu-restricted-extras vdpau-va-driver vim vim-gtk vlc vlc-plugin-pulse zlib1g-dev" #g15daemon tvtime me-tv eclipse texmaker texlive"

APPZ_ALTRI_REPO="$WXXCODECS libdvdcss2 jdownloader faenza-icon-theme synapse indicator-multiload indicator-sensors my-weather-indicator"

APPZ_DIPENDENZE_ECLIPSE="ant ant-optional default-jdk fastjar gcj-4.6-base gcj-4.6-jre-lib jarwrapper junit junit4 libapache-pom-java libasm3-java libcommons-beanutils-java libcommons-cli-java libcommons-codec-java libcommons-collections3-java libcommons-compress-java libcommons-digester-java libcommons-el-java libcommons-httpclient-java libcommons-lang-java libcommons-logging-java libcommons-parent-java libdb-java libdb-je-java libdb5.1-java libdb5.1-java-gcj libecj-java libequinox-osgi-java libgcj-bc libgcj-common libgcj12 libhamcrest-java libice-dev libicu4j-4.4-java libicu4j-java libjasper-java libjetty-java libjline-java libjsch-java libjtidy-java liblucene2-java libpthread-stubs0 libpthread-stubs0-dev libregexp-java libservlet2.4-java libslf4j-java libsm-dev libswt-cairo-gtk-3-jni libswt-glx-gtk-3-jni libswt-gnome-gtk-3-jni libswt-gtk-3-java libswt-gtk-3-jni libswt-webkit-gtk-3-jni libx11-dev libx11-doc libxau-dev libxcb1-dev libxdmcp-dev libxt-dev openjdk-6-jdk sat4j x11proto-core-dev x11proto-input-dev x11proto-kb-dev xorg-sgml-doctools xtrans-dev"

APPZ="$APPZ_REPO_UFFICIALI $APPZ_ALTRI_REPO $APPZ_DIPENDENZE_ECLIPSE"
...
PARTNER="deb http://archive.canonical.com/ubuntu precise partner"
PARTNER_SRC="deb-src http://archive.canonical.com/ubuntu precise partner"
PARTNER_SED='sed -i -e "s/# deb http:\/\/archive.canonical.com\/ubuntu precise partner/deb http:\/\/archive.canonical.com\/ubuntu precise partner/g" /etc/apt/sources.list'
PARTNER_SRC_SED='sed -i -e "s/# deb-src http:\/\/archive.canonical.com\/ubuntu precise partner/deb-src http:\/\/archive.canonical.com\/ubuntu precise partner/g" /etc/apt/sources.list'

BACKPORTS="deb http://it.archive.ubuntu.com/ubuntu/ precise-backports main restricted universe multiverse"
BACKPORTS_SRC="deb-src http://it.archive.ubuntu.com/ubuntu/ precise-backports main restricted universe multiverse"
BACKPORTS_SED='sed -i -e "s/# deb http:\/\/it.archive.ubuntu.com\/ubuntu\/ precise-backports main restricted universe multiverse/deb http:\/\/it.archive.ubuntu.com\/ubuntu\/ precise-backports main restricted universe multiverse/g" /etc/apt/sources.list'
BACKPORTS_SRC_SED='sed -i -e "s/# deb-src http:\/\/it.archive.ubuntu.com\/ubuntu\/ precise-backports main restricted universe multiverse/deb-src http:\/\/it.archive.ubuntu.com\/ubuntu\/ precise-backports main restricted universe multiverse/g" /etc/apt/sources.list'
...
enable_repo "Ubuntu Precise-Backports" "$BACKPORTS" "$BACKPORTS_SED"
enable_repo "Ubuntu Precise-Backports (Sorgenti)" "$BACKPORTS_SRC" "$BACKPORTS_SRC_SED"
...
add_repo "google-chrome" "$GOOGLE_CHROME" "$GOOGLE_KEY_KEY"
add_repo "google-talkplugin" "$GOOGLE_TALKPLUGIN" "$GOOGLE_KEY_KEY"
...
apt-get remove $AUTOYES postfix
