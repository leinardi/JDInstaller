...
APPZ_REPO_UFFICIALI="avidemux banshee banshee-community-extensions build-essential bwm-ng cmatrix compizconfig-settings-manager conky deluge-torrent emesene ffmpeg filezilla finch fusion-icon gimp gmail-notify gnome-art gnome-commander gnome-mplayer gparted gthumb gtk-recordmydesktop hardinfo hddtemp htop iotop k3b lm-sensors mc meld mercurial mplayer msttcorefonts mumble nautilus-actions nautilus-image-converter nautilus-open-terminal nfs-common nfs-kernel-server openoffice.org p7zip p7zip-full p7zip-rar padevchooser pidgin powertop pwgen rar sensors-applet sloccount skype smartmontools soundconverter sox startupmanager subtitleeditor subversion sun-java6-plugin tagtool tofrodos tree ubuntu-restricted-extras vdpau-va-driver vim vim-gtk vlc vlc-plugin-pulse zlib1g-dev" # g15daemon tvtime me-tv eclipse texmaker texlive"

APPZ_ALTRI_REPO="$WXXCODECS libdvdcss2 ubuntu-tweak jdownloader avant-window-navigator-trunk avant-window-navigator-data-trunk python-awn-trunk awn-settings-trunk awn-applets-python-core-trunk python-awn-extras-trunk awn-applets-python-extras-trunk awn-applets-c-core-trunk awn-applets-c-extras-trunk"
...
PARTNER="deb http://archive.canonical.com/ubuntu maverick partner"
PARTNER_SRC="deb-src http://archive.canonical.com/ubuntu maverick partner"
PARTNER_SED='sed -i -e "s/# deb http:\/\/archive.canonical.com\/ubuntu maverick partner/deb http:\/\/archive.canonical.com\/ubuntu maverick partner/g" /etc/apt/sources.list'
PARTNER_SRC_SED='sed -i -e "s/# deb-src http:\/\/archive.canonical.com\/ubuntu maverick partner/deb-src http:\/\/archive.canonical.com\/ubuntu maverick partner/g" /etc/apt/sources.list'

BACKPORTS="deb http://it.archive.ubuntu.com/ubuntu/ maverick-backports main restricted universe multiverse"
BACKPORTS_SRC="deb-src http://it.archive.ubuntu.com/ubuntu/ maverick-backports main restricted universe multiverse"
BACKPORTS_SED='sed -i -e "s/# deb http:\/\/it.archive.ubuntu.com\/ubuntu\/ maverick-backports main restricted universe multiverse/deb http:\/\/it.archive.ubuntu.com\/ubuntu\/ maverick-backports main restricted universe multiverse/g" /etc/apt/sources.list'
BACKPORTS_SRC_SED='sed -i -e "s/# deb-src http:\/\/it.archive.ubuntu.com\/ubuntu\/ maverick-backports main restricted universe multiverse/deb-src http:\/\/it.archive.ubuntu.com\/ubuntu\/ maverick-backports main restricted universe multiverse/g" /etc/apt/sources.list'
...
enable_repo "Ubuntu Maverick-Backports" "$BACKPORTS" "$BACKPORTS_SED"
enable_repo "Ubuntu Maverick-Backports (Sorgenti)" "$BACKPORTS_SRC" "$BACKPORTS_SRC_SED"
