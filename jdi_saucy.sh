...
APPZ_REPO_UFFICIALI="build-essential bwm-ng cmatrix compizconfig-settings-manager conky dkms ffmpeg filezilla finch flashplugin-installer gdebi gimp git gnome-backgrounds gnome-commander gnome-mplayer gnome-search-tool gnome-session-fallback gnome-system-tools gnome-themes-standard gnome-tweak-tool gparted gthumb gtk-recordmydesktop hardinfo hddtemp htop icedtea-plugin inkscape iotop jstest-gtk k3b libreoffice lm-sensors linux-headers-generic mc meld mercurial mplayer mumble nautilus-actions nautilus-dropbox nautilus-image-converter nautilus-open-terminal nfs-common nfs-kernel-server p7zip p7zip-full p7zip-rar pidgin powertop pavucontrol pwgen rar sloccount skype smartmontools soundconverter sox subtitleeditor subversion synapse synaptic tagtool tofrodos tree ttf-mscorefonts-installer ubuntu-restricted-extras unity-tweak-tool vdpau-va-driver vim vim-gtk vlc vlc-plugin-pulse zlib1g-dev" #g15daemon tvtime me-tv eclipse texmaker texlive" #myunity

APPZ_ALTRI_REPO="libdvdcss2 jdownloader indicator-multiload my-weather-indicator google-chrome-beta google-musicmanager-beta google-talkplugin" # indicator-sensors
...
PARTNER="deb http://archive.canonical.com/ubuntu saucy partner"
PARTNER_SRC="deb-src http://archive.canonical.com/ubuntu saucy partner"
PARTNER_SED='sed -i -e "s/# deb http:\/\/archive.canonical.com\/ubuntu saucy partner/deb http:\/\/archive.canonical.com\/ubuntu saucy partner/g" /etc/apt/sources.list'
PARTNER_SRC_SED='sed -i -e "s/# deb-src http:\/\/archive.canonical.com\/ubuntu saucy partner/deb-src http:\/\/archive.canonical.com\/ubuntu saucy partner/g" /etc/apt/sources.list'

BACKPORTS="deb http://it.archive.ubuntu.com/ubuntu/ saucy-backports main restricted universe multiverse"
BACKPORTS_SRC="deb-src http://it.archive.ubuntu.com/ubuntu/ saucy-backports main restricted universe multiverse"
BACKPORTS_SED='sed -i -e "s/# deb http:\/\/it.archive.ubuntu.com\/ubuntu\/ saucy-backports main restricted universe multiverse/deb http:\/\/it.archive.ubuntu.com\/ubuntu\/ saucy-backports main restricted universe multiverse/g" /etc/apt/sources.list'
BACKPORTS_SRC_SED='sed -i -e "s/# deb-src http:\/\/it.archive.ubuntu.com\/ubuntu\/ saucy-backports main restricted universe multiverse/deb-src http:\/\/it.archive.ubuntu.com\/ubuntu\/ saucy-backports main restricted universe multiverse/g" /etc/apt/sources.list'
...
GETDEB="deb http://archive.getdeb.net/ubuntu saucy-getdeb apps"
...
enable_repo "Ubuntu Saucy-Backports" "$BACKPORTS" "$BACKPORTS_SED"
enable_repo "Ubuntu Saucy-Backports (Sorgenti)" "$BACKPORTS_SRC" "$BACKPORTS_SRC_SED"
